...
## 1. Subtitles (OpenSubtitles) - PRIORITY

- **Configuration**: Add `OPENSUBTITLES_API_KEY` to `server/config/config.go`.
- **Database & Models**: 
    - Update `server/database/migrations.go` to add `imdb_id` to `movies`, `shows`, and `requests` tables (required for accurate subtitle matching).
    - Update `server/models/request.go`, `server/models/movie.go`, and `server/models/show.go` to include `IMDBID`.
...
- **UI**: Add a "Search Subtitles" button to Movie and Show details pages.

## 2. Core Download Infrastructure
...
## 3. Custom Torrent Search Services
